...
// Ініціалізація поля з рівномірними деревами
void initField() {
    srand(time(0));

    int gridSizeX = 8; // горизонтальний проміжок між деревами
    int gridSizeY = 6; // вертикальний проміжок між деревами

    for (int i = 2; i < HEIGHT-3; i += gridSizeY) {
        for (int j = 2; j < WIDTH-3; j += gridSizeX) {
            int offsetX = rand() % 3; // невеликий випадковий зсув
            int offsetY = rand() % 3;
...
void drawField(int glowPhase){
    clear();
    // спочатку заповнюємо все крапочками
...
// Ріст яблук – покращена версія
void growApples(){
    static int growCounter = 0;
    growCounter++;
    if(growCounter>=3){ // кожні 3 цикли
        for(int i=1;i<HEIGHT-1;i++){
            for(int j=1;j<WIDTH-1;j++){
                if(field[i][j].isTree && field[i][j].appleStage<2){
                    if(rand()%2==0) field[i][j].appleStage++; // 50% шанс росту
                }
            }
        }
        growCounter = 0;
...
int main(){
    ...
    // кольори
    init_pair(1,COLOR_CYAN,COLOR_BLACK);   // гравець
    init_pair(2,COLOR_YELLOW,COLOR_BLACK); // стовбур
    init_pair(3,COLOR_GREEN,COLOR_BLACK);  // крона
    init_pair(4,COLOR_YELLOW,COLOR_BLACK); // молоде яблуко
    init_pair(5,COLOR_RED,COLOR_BLACK);    // зріле яблуко блимає
